...
import rotateVid from "/videos/a.mp4";
...
const LoginPage = () => {
  ...
  const { signInWithGoogle, signInWithPassword, requestPasswordReset } =
    useAuth();
  ...
  const handleGoogleLogin = async () => {
    ...
    setInfo(null);
    try {
      await signInWithGoogle(PATHS.root);
    } catch (err) {
      setError(
  ...
  const handleForgotPassword = async () => {
    ...
    const { error: resetError } = await requestPasswordReset(email, PATHS.login);
    if (resetError) {
      setError(resetError);
      return;
    }
            ...
            <form onSubmit={handleLogin} className="max-w-md mx-auto">
              ...
              <div className="flex items-center justify-center mb-8">
                <img
                  ...
                  onClick={() => navigate("/")}
                  alt="Logo"
                  className="h-20 cursor-pointer hover:scale-[1.1] transition-transform ease-in-out duration-500"
                />
              </div>
              {/* Heading */}
